let now =
  Number(localStorage.getItem("currentActiveUserTime")) + 70 * 1000 || 0;

let intervalID = setInterval(() => {
  let diff: number = now - Date.now();

  let minutes: string | number = Math.floor(diff / (1000 * 60));
...
let UserActivityDetails = JSON.parse(
  localStorage.getItem("UserActivityDetails") || "[]"
);

let pushOk = true;
UserActivityDetails.forEach((user: { name: string }) => {
...
if (pushOk == true) {
  ...
  localStorage.setItem(
    "UserActivityDetails",
    JSON.stringify(UserActivityDetails)
  );
}
...
function updateLocalStorage() {
  ...
  UserActivityDetails = JSON.parse(
    localStorage.getItem("UserActivityDetails") || "[]"
  );
}

document.getElementById("welcome")!.innerText = `Hey ${localStorage.getItem(
  "currentActiveUser"
)}, Welcome Back!`;
...
document.getElementById(
  "fundMoney"
)!.innerText = `Total fund has:  ${UserActivityDetails[indeX].fundMoney}`;
document.getElementById("addMoneyBtn")!.addEventListener("click", (e) => {
  let value = (document.getElementById("addMoneyInput") as HTMLInputElement)
    .value;
  (document.getElementById("addMoneyInput") as HTMLInputElement).value = "";
  if (Number(value) <= 0) {
    ...
    (
      document.getElementById("fundMoney") as HTMLInputElement
    ).innerText = `Total fund has:  ${UserActivityDetails[indeX].fundMoney}`;
...
function showTotalSpendsOnShopping() {
  document.getElementById(
    "totalSpendOnShopping"
  )!.innerText = `Total Spends on Shopping: ${UserActivityDetails[indeX].totalSpendOnShopping}`;
}
...
function fetchingCard() {
  ...
  UserActivityDetails[indeX].card.forEach(
    (card: { price: number; id: number; name: string }) => {
...
document.getElementById("addCard")!.addEventListener("click", () => {
  if (
    ...
    (document.getElementById("cardPrice") as HTMLInputElement).value
  ) {
    UserActivityDetails[indeX].card.push({
      name: (document.getElementById("cardName") as HTMLInputElement).value,
      price: (document.getElementById("cardPrice") as HTMLInputElement).value,
      id: Math.random() + 100,
    });
...
function fetichingLast5activities() {
  ...
  let c = 1;
  UserActivityDetails[indeX].activities.forEach(
    (d: { details: string; timestamp: string }) => {
      document.getElementById("activities")!.innerHTML += `
  <div style =" ">
